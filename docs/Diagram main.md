...
On Modern Scripts, any qualifying derivation is shown with dotted lines if reasonably disputed in academic sources (or there are few sources and they are all hesitant). If this dispute would affect the designated group (colour) of the script, then that colour is determined in the same way as in the All Scripts chart with one exception.
