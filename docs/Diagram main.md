...
# Minor notes
  ...
  - While researching, there were two big gotcha's to be aware of: Many scripts have the same name as the corresponding language, so had to be careful it was specifically the script that was being mentioned in the source. Second, for related scripts, sources would mention an Old or Proto script, eg. proto-Bengali, proto-Odiya, etc., based on whichever script they were talking about. However it's important to note that sometimes these different names are actually referring to essentially the exact same script before the child scripts became distinct scripts.
